...
async def _migrate_columns_sqlite(conn):
    dialect = conn.dialect.name
    if dialect == "sqlite":
        for table in ["consumption"]:
            res = await conn.execute(text(f"PRAGMA table_info({table})"))
            existing = {row[1] for row in res.fetchall()}
            for col in ["cubic_meters", "kg_per_m3", "project_name", "concrete_grade"]:
                if col not in existing:
                    col_type = "FLOAT" if col in ["cubic_meters","kg_per_m3"] else "TEXT"
...
async def get_receipts(session, user_id=None, limit=20):
    q = select(CementReceipt).order_by(CementReceipt.timestamp.desc()).limit(limit)
...
async def get_consumptions(session, user_id=None, limit=20):
    q = select(CementConsumption).order_by(CementConsumption.timestamp.desc()).limit(limit)
...
async def add_audit_log(session, telegram_id, action, details=None, user_id=None):
    ...
    await session.commit()
async def get_audit_logs(session, limit=30):
    return (await session.execute(select(AuditLog).order_by(AuditLog.timestamp.desc()).limit(limit))).scalars().all()
